...
PixelShader =
{
	...
	[[
		...
		{
			float waterHeight = MultiSampleTexX( HeightTexture, Input.uv );
			waterHeight /= ( 95.7f / 255.0f );
			float waterShore = waterHeight;
			waterHeight = saturate( ( waterHeight - 0.995f ) * 50.0f );
			waterShore = saturate( ( waterShore - 0.962f ) * 85.0f );
